...
하지만 이렇게 얻은 장점 뒤에는 단점 또한 존재합니다. TLB miss 시 조회(a.k.a 메모리 룩업)가 최악의 경우 24번 정도 난다는 겁니다. 왜 최대 24번인지? 제가 처음에 의문이 들었기에 그 내용을 간단하게 짚고 넘어가겠습니다. 
...
사실 생각보다 간단합니다. 게스트 페이지 테이블은 과연 어디에 저장될까요? 당연히 메모리 어딘가에 저장 되어있습니다. 가상화 환경은 보통 자신이 가상화 환경을 모르도록 설계가 되어있기에 이러한 테이블도 당연히 GPA 위치로 저장되어 있습니다. 이런 GPA위치를 실제 호스트 메모리 위치를 가르키려면? 당연히 HPA가 필요합니다.

그럼 여기서 다시 한번 살펴보면

* 페이지 테이블은 메모리에 저장되어 있다.
* 게스트 페이지 테이블은 GPA에 저장된다.
* GPA는 HPA로 변환 작업이 필요하다.

여기서 결론은? "게스트 페이지 테이블 4개에서 각 테이블 참조마다 호스트 페이지 테이블 4개를 거쳐야 실제 주소가 나온다." 입니다.

그럼 여기서 4 * 4 니까 16 아닌가? 라는 생각이 드는데 
