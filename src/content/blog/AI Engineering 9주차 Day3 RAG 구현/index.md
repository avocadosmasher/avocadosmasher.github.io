...
위의 구성 요소들은 실무에서는 사실 직접 만들게 되는 경우가 많지 않다고 합니다. 실제 서비스시에는  잘 만들어진 라이브러리들이 있어서 사실 가져다가 잘 사용만 하면 되며, 미니 프로젝트의 구성 요소와 실제 서비스에서 사용할만한 라이브러리를 맵핑한 정보는 대강 아래 테이블과 같습니다.
...
> 테스트로 `documents` 의 첫 번째 요소인 `RAG 개요`문서의 `text`부분을 간단하게 넣어서 하나씩 돌려가며 흐름을 짚어보고자 합니다.
...
// 테스트 구문
test_text = normalize_text(documents[0].get("text"))
print(test_text)
```

결과를 보면 띄어쓰기로 구분된 문자열을 받을 수 있습니다.
```python
'RAG는 Retrieval-Augmented Generation의 약자로 검색 증강 생성을 의미한다. LLM이 모든 지식을 내부 파라미터에만 의존하지 않고,...
```


`split_sentences`함수는 전체 텍스트를 문장 단위로 변환하기 위해서 일반적으로 문장의 끝을 나타내는 특수 문자들 단위로 나누는 함수입니다.
***p.s 요즘은 regex 문법 같은 것도 직접 구현할 필요 없이 AI를 통해서 빠르게 구현이 되니까 좋은 것 같습니다.***
```python
...
// 테스트 구문
test_text_list = split_sentences(nom_text)
print(test_text_list)
```

아래와 같이 특수문자 `.?!`로 구분된 문장들이 담긴 리스트를 반환합니다.
```python
['RAG는 Retrieval-Augmented Generation의 약자로 검색 증강 생성을 의미한다.',
'LLM이 모든 지식을 내부 파라미터에만 의존하지 않고, 외부 문서에서 관련 정보를 검색한 뒤 답변을 생성하는 방식이다.',
'RAG는 최신 정보 반영, 출처 제공, 도메인 지식 활용, hallucination 완화에 도움이 된다.',
...
// 테스트 구문
print(sentence_chunker(documents[0]["text"]))
```

default 값으로 2문장씩 chunking 하는데 overlap을 1개를 줘서 chunk당 2문장 그리고 1문장씩 겹치게 됩니다.
```python
  
['RAG는 Retrieval-Augmented Generation의 약자로 검색 증강 생성을 의미한다. LLM이 모든 지식을 내부 파라미터에만 의존하지 않고, 외부 문서에서 관련 정보를 검색한 뒤 답변을 생성하는 방식이다.',
'LLM이 모든 지식을 내부 파라미터에만 의존하지 않고, 외부 문서에서 관련 정보를 검색한 뒤 답변을 생성하는 방식이다. RAG는 최신 정보 반영, 출처 제공, 도메인 지식 활용, hallucination 완화에 도움이 된다.',
'RAG는 최신 정보 반영, 출처 제공, 도메인 지식 활용, hallucination 완화에 도움이 된다. 일반적인 RAG 파이프라인은 문서 수집, 청킹, 임베딩, 벡터 저장, 검색, 답변 생성 단계로 구성된다.']
```
...
최종 결과물의 함수 흐름을 보면 아래와 같습니다. 그렇게 복잡한 부분은 없습니다 ― chunking에서 복잡한 기술을 사용하지 않았기 때문에 비교적 간단하게 구현되었습니다. 나중에 chunking 기술들도 공부해서 글을 적어보겠습니다.
![rag_chunking_function_call_chain](./rag_chunking_function_call_chain.png)

### Simple TF-IDF 임베딩 구현
미니 프로젝트에서는 일반적으로 사용하는 임베딩 모델(`SentenceTransformer`) 대신해 직접 간단하게 TF-IDF 임베딩을 구현합니다.

### TF-IDF란?
우선 구현하기 전에 TF-IDF가 뭔지 간단하게 조사해서 공부하고 진행하겠습니다.
TF-IDF의 핵심은 **"이 문서를 대표하는 단어는 뭔가?"** 에 답하기 위한 두 가지 상식을 곱한 것입니다.

각각의 TF, IDF가 뜻하는 바는 아래와 같습니다.
* ***TF(Term Frequency)** : 이 문서에서 자주 나오면 중요하다.
	* "검색"이 10번 나온 문서는 1번 나온 문서보다 검색에 대한 문서일 가능성이 높죠.
* ***IDF(Inverse Document Frequency)** : 모든 문서에 나오는 단어는 쓸모없다.
	* "그리고", "합니다"는 어느 문서에나 자주 나옵니다. TF만 보면 이런 단어가 1등 차지하게 됩니다. 그래서 **"전체 문서 중 몇 개에 등장하는가(DF)"의 역수**를 곱해서 흔한 단어의 가중치를 깎아내립니다.

핵심은 TF와 IDF를 곱한다는 데 있습니다. 둘 중 하나라도 0에 가까우면 점수가 죽습니다. 즉 생존한 단어는 **"이 문서에는 자주 나오는데, 다른 문서에는 잘 안 나오는"** 단어 뿐이며, 그게 바로 이 문서를 다른 문서와 구별해주는 단어입니다. 
![tfidf_score_decomposition_bars](./tfidf_score_decomposition_bars.png)
하지만 이렇게 값만 곱하고 끝난다면 가중치의 차이가 너무 커지기에 log를 씌워 그 값의 차이를 줄이게 됩니다.

전 통계학이나 수학을 전공하지는 않았기에 여기서 한 가지 의문이 들었었는데요 "왜 하필이면 로그를 씌운걸까?"라는 의문이 들었고 그 이유를 찾아보기로 했습니다.
#### TF x IDF 에 Log를 씌우는 이유


하지만 한계점이 명확합니다. 단어의 빈도 통계에 의존한 임베딩 모델로 단어의 동의어 구분, 문맥 이해 같은게 불가하여 semantic 검색이나 문장/문서 임베딩 보다는 **키워드 추출, 문서 유사도의 간단한 근사치 비교** 같은 곳에 쓰입니다.



사실 실무에서는 직접 임베딩 구현까지 하게 될 일이 많지 않을 거라 ― 잘 만들어진걸 가져다 사용하니...  ― 코드보다는 큰 흐름을 이해하는게 중요하다고 생각이 드네요  ― 어차피 필요하면 그때 가서 또 찾아봐야 합니다. 
...
우선 토큰화를 진행할 문자열 셋의 패턴과 접미사의 패턴을 아래와 같이 정의했습니다. 
`긴 접미사 -> 짧은 접미사` 순서로 정렬되도록 했는데요, 간혹 아래와 같은 문제가 발생해 이를 방지하기 위함입니다.
***예시 : "인공지능이라는" → "라는" 먼저 매칭 → "인공지능이"   ← 어간에 '이'가 남음***
```python
TOKEN_PATTERN = re.compile(r"[가-힣A-Za-z0-9_]+")
KOREAN_SUFFIXES = tuple(sorted((
...
여기서 확인하고 넘어갈 점은 떼어내고 남는 어근을 최소 2글자 이상 남긴다는 점입니다.
<details>
<summary ><b>어근을 최소 2글자 이상 남기는 이유</b></summary>
...
보시는 것 처럼 "문자열에서 접미사를 뗀 글자가 2글자 이상이다." 라는 것을 좀 더 직관적으로 확인이 가능하죠.

이게 형태소 사전을 사용하지 않고 있기 때문에 오분할(false splitting)을 막기 위한 일종의 안전장치인데요 ― 한국어에서는 **2음절** 명사가 조사와 같은 글자로 끝나는 경우가 굉장히 많기 때문입니다.

예를 들어서  `사과` 라는 단어가 있다고 치면 `과`도 사실 접미사중 하나가 있지만 `사과`는 그냥 명사 `사과`잖아요? 저런식의 단어가 꽤~~~ 많아서 저런 오분할을 막기 위함입니다.
</details>
