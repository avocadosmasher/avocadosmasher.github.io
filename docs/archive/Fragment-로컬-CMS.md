...
외부 계정·비용·OAuth 연결 순서는 [외부 인증 연결 안내](Fragment-외부-인증.md)를 참고한다. A04-1에서 [OAuth Worker와 테스트 CMS](../../workers/fragment-oauth/README.md)의 로컬 검증을 마쳤으며 실제 계정 연결은 A04-2에서 진행한다. 아래 proxy 실행과 OAuth 테스트 모드는 별개다.
