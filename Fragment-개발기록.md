...
## G01-UI — 블로그 전용 작성 폼 (2026-09-15, 수동 판정 대기)
...
- 코드·설정·로컬 검증은 준비했다. 계정에서 필요한 설정을 연결한 뒤 테스트 환경에 배포하고 실제 로그인 → 샘플 커밋 → 다른 브라우저 세션 읽기·수정·ID 보존을 인수한다. 계정 설정 전에는 인증 성공으로 표시하지 않는다.
