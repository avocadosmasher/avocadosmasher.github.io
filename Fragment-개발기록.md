...
## 다음 재개 지점 — A03-4: 외부 인증 구성 준비

- OAuth 제공자 후보와 비용·권한·콜백·필요 계정 설정을 공식 자료로 확인하고 비교·연결 안내를 작성한다. 사용자는 Cloudflare/Netlify 계정이 아직 없다.
- 이 작업은 A03의 남은 외부 구성 조사 범위를 작은 태스크로 나눈 것이다. 이후 A04의 실제 로그인 → 테스트 저장소 커밋 → 다른 세션 읽기 인수를 진행한다. 문서 준비만으로 실제 인증/저장 완료로 표시하지 않는다.
