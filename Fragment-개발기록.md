...
## 다음 재개 지점 — A04-1: OAuth 중계 구현·로컬 검증

- 외부 인증 안내의 Cloudflare 권장안을 기준으로 Worker의 auth/callback과 별도 테스트 CMS 설정을 구현한다. state·PKCE·허용 origin·오류 응답·비밀값 비노출을 실패 테스트부터 검증한다. 계정 설정 전에도 로컬 작업은 가능하다.
- 한 턴에 이 작은 태스크 하나를 검증·기록한다. 이후 A04-2에서 계정·테스트 저장소·관리 화면 주소가 준비되면 테스트 환경 배포 → 실제 로그인 → 커밋 → 다른 세션 읽기를 인수한다.
- 사용자는 Cloudflare/Netlify 계정이 아직 없다고 한 상태다. URL과 Secret 설정 완료 여부만 공유하며 비밀값은 대화·저장소에 넣지 않는다.
