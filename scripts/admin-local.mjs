...
const sandbox = path.join(project, '.fragment-test', 'cms');
...
if ((await realpath(sandbox)).toLowerCase() !== path.resolve(sandbox).toLowerCase()) {
  throw new Error('CMS sandbox must be a real directory inside .fragment-test/cms');
}
...
start('node_modules/astro/astro.js', ['dev', '--host', '127.0.0.1', '--port', '4400'], project, {
  FRAGMENT_CMS_LOCAL: '1',
});
console.log('Local CMS: http://127.0.0.1:4400/admin/ — data: .fragment-test/cms');
